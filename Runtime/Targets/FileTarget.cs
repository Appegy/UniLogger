...
namespace UnityEngine
{
    ...
    internal class FileTarget : Target
    {
        ...
        public FileTarget(LoggerConfig config, Formatter formatter, string logFilePath, long fileSizeLimit, TimeSpan liveTime, int bufferSize = 1024, bool autoFlush = false)
        ...
        {
            var sizeOfLine = log.Length; // because sizeof(char) for UTF8 encoding is 1 byte. UTF-16 encoding is used by default in .NET
            if (_currentFileSize + sizeOfLine > _fileSizeLimit)
            {
                Flush();
                CreateLogFile();
